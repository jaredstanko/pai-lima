...
echo -e "     ${CYAN}and verify the voice server can successfully generate and play audio${NC}"
echo -e "     ${CYAN}end-to-end (not just that the process is listening). Fix any${NC}"
echo -e "     ${CYAN}macOS-specific binaries (like afplay) that won't work on Linux.${NC}"
echo -e "     ${CYAN}Set both to start on boot.${NC}"
echo ""
